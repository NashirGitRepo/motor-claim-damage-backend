...
export class idsutil {
  ...
  async getIDSClientInstance(parentSpanInst, clientInstance = null, ...others) {
    ...
    try {
      ...
      bh = await this.sd_0e7fYLnmZdXkm9hC(bh, parentSpanInst);
  ...
  async sd_0e7fYLnmZdXkm9hC(bh, parentSpanInst) {
    const spanInst = this.tracerService.createSpan(
      'sd_0e7fYLnmZdXkm9hC',
      parentSpanInst
    );
    try {
      ...
      //appendnew_next_sd_0e7fYLnmZdXkm9hC
      return bh;
    } catch (e) {
      return await this.errorHandler(
        bh,
  ...
  async sd_NDvR3YN7qjsaaO0E(bh, parentSpanInst) {
    ...
    try {
      if (
        ...
        bh = await this.sd_UMnO5NVIRpn584Hd(bh, parentSpanInst);
      }
      this.tracerService.sendData(spanInst, bh);
  ...
  async sd_J9L3oEbqPtl8tfbi(bh, parentSpanInst) {
    ...
    try {
      ...
      this.tracerService.sendData(spanInst, bh);
      bh = await this.sd_NzkxynHEriEIBGME(bh, parentSpanInst);
  ...
  async sd_NzkxynHEriEIBGME(bh, parentSpanInst) {
    const spanInst = this.tracerService.createSpan(
      'sd_NzkxynHEriEIBGME',
      parentSpanInst
    );
    try {
      ...
      this.tracerService.sendData(spanInst, bh);
      //appendnew_next_sd_NzkxynHEriEIBGME
      return bh;
    } catch (e) {
      return await this.errorHandler(
        bh,
  ...
  async sd_UMnO5NVIRpn584Hd(bh, parentSpanInst) {
    const spanInst = this.tracerService.createSpan(
      'sd_UMnO5NVIRpn584Hd',
      parentSpanInst
    );
    try {
      ...
      this.tracerService.sendData(spanInst, bh);
      //appendnew_next_sd_UMnO5NVIRpn584Hd
      return bh;
    } catch (e) {
      return await this.errorHandler(
        bh,
  ...
  private async errorHandler(
    ...
    if (
      false ||
      (await this.sd_G1KN2w3rJCIQ4Wlm(bh, parentSpanInst))
  ...
  async sd_G1KN2w3rJCIQ4Wlm(bh, parentSpanInst) {
    const nodes = ['handleTokenExpiry'];
    if (nodes.includes(bh.errorSource)) {
      bh = await this.sd_EgUC5B8p6Vp4y70s(bh, parentSpanInst);
      //appendnew_next_sd_G1KN2w3rJCIQ4Wlm
      return true;
    }
    return false;
  }
